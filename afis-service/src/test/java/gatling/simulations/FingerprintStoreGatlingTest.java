...
public class FingerprintStoreGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all fingerprintStores")
                    .get("/services/afisservice/api/fingerprint-stores")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new fingerprintStore")
                        .post("/services/afisservice/api/fingerprint-stores")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_fingerprintStore_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created fingerprintStore")
                            .get("/services/afisservice${new_fingerprintStore_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created fingerprintStore")
                        .delete("/services/afisservice${new_fingerprintStore_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FingerprintStore entity").exec(scn);
