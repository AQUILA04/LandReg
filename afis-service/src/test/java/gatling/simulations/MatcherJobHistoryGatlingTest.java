...
public class MatcherJobHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all matcherJobHistories")
                    .get("/services/afisservice/api/matcher-job-histories")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new matcherJobHistory")
                        .post("/services/afisservice/api/matcher-job-histories")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"rid\": \"SAMPLE_TEXT\"" +
                                ", \"producerCount\": 0" +
                                ", \"consumerReponseCount\": 0" +
                                ", \"highScore\": 0" +
                                ", \"foundMatch\": null" +
                                ", \"matchedRID\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_matcherJobHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created matcherJobHistory")
                            .get("/services/afisservice${new_matcherJobHistory_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created matcherJobHistory")
                        .delete("/services/afisservice${new_matcherJobHistory_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MatcherJobHistory entity").exec(scn);
