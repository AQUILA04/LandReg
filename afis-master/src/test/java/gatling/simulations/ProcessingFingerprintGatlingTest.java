...
/**
 * Performance test for the ProcessingFingerprint entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.7.3/generators/gatling#logging-tips">Logging tips</a>
 */
public class ProcessingFingerprintGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all processingFingerprints")
                    .get("/services/afismaster/api/processing-fingerprints")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new processingFingerprint")
                        .post("/services/afismaster/api/processing-fingerprints")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_processingFingerprint_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created processingFingerprint")
                            .get("/services/afismaster${new_processingFingerprint_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created processingFingerprint")
                        .delete("/services/afismaster${new_processingFingerprint_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ProcessingFingerprint entity").exec(scn);
